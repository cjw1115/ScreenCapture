...
void NativeGraghic::GetCanvasDevice(ABI::Windows::Graphics::DirectX::Direct3D11::IDirect3DDevice** canvasDevice)
{
	HSTRING hstring;
	std::wstring name = L"Microsoft.Graphics.Canvas.CanvasDevice";
	WindowsCreateString(name.c_str(), name.length(), &hstring);
	HRESULT hr = RoActivateInstance(hstring, (IInspectable**)canvasDevice);
	assert(hr == S_OK);
